library(grDevices)
AHe=read.table("/Users/smoron/Documents/1.Projects/Thermochron/Thermochron_figs&tables_0819/Table_3_1_selected.csv", sep=',', header=T)
names(AHe)
#Formatting
tma=.03
tmi=.01
samples=levels(AHe$Sample_No); length(samples)
...
#AHe vs eU#
###########
pdf("/Users/smoron/Documents/1.Projects/Thermochron/Thermochron_figs&tables_0819/AHevseU_uncorrected_colorcoded_0819_selected.pdf", width=15/2.54, height=15/2.54)
par(mfrow=c(1, 2))
par(mar=c(2,2,0,0))


xlim= c(min(AHe$eU_ppm), max(AHe$eU_ppm)+40)
ylim=c(min(AHe$Uncorrected_Age_Ma)-20, max(AHe$Uncorrected_Age_Ma))
